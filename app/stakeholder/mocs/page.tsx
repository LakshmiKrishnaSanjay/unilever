...
export default function StakeholderMOCsPage() {
  ...
  useEffect(() => {
    ...
    // Filter MOCs relevant to stakeholder (requiring stakeholder approval)
    const stakeholderMocs = allMocs.filter((m) => m.requiresStakeholderApproval);
    
    // Sort by most recent first
    stakeholderMocs.sort((a, b) => {
      const dateA = new Date(a.createdAt).getTime();
      const dateB = new Date(b.createdAt).getTime();
      return dateB - dateA;
    });

    setMocs(stakeholderMocs);
    setLoading(false);
  }, []);
    ...
    <DashboardLayout>
      <div className="space-y-6">
        <div className="flex items-center justify-between">
          <div>
            <h1 className="text-3xl font-semibold">My MOCs</h1>
            <p className="text-muted-foreground">
              View MOCs requiring your approval
            </p>
          </div>
        </div>
                ...
                <TableBody>
                  {filteredMOCs.length === 0 ? (
                    ...
                    filteredMOCs.map((moc) => {
                      const isApproved = moc.status === 'APPROVED' || moc.status === 'STAKEHOLDER_APPROVED';
