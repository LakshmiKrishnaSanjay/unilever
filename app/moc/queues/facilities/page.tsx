...
export default function FacilitiesMOCQueuePage() {
  const { mocs = [] } = useWorkflow();

  const pendingMOCs = mocs.filter((m) => m.status === 'PENDING_FACILITIES_REVIEW');

  return (
    <DashboardLayout>
      <div className="space-y-6">
        <div>
          <h1 className="text-3xl font-semibold">Facilities MOC Review Queue</h1>
          <p className="text-muted-foreground">MOC Pack reviews pending your approval</p>
        </div>
          ...
          <CardContent>
            {pendingMOCs.length === 0 ? (
              <div className="text-center py-12">
                ...
                <p className="mt-4 text-muted-foreground">No MOCs pending review</p>
              </div>
            ) : (
              <Table>
                <TableHeader>
