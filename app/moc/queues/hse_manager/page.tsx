...
export default function HSEManagerMOCQueuePage() {
  const { mocs = [] } = useWorkflow();

  const pendingMOCs = mocs.filter((m) => m.status === 'PENDING_HSE_APPROVAL');

  return (
    <DashboardLayout>
      <div className="space-y-6">
        <div>
          <h1 className="text-3xl font-semibold">HSE Manager MOC Review Queue</h1>
          <p className="text-muted-foreground">MOCs pending final HSE approval</p>
        </div>
          ...
          <CardContent>
            {pendingMOCs.length === 0 ? (
              <div className="text-center py-12">
                ...
                <p className="mt-4 text-muted-foreground">No MOCs pending final approval</p>
              </div>
            ) : (
              <Table>
                <TableHeader>
